...
The best way to run the code would be to open the .Rproj first. This will ensure that `source("setup.R")` will find the "source.R" script in its home directory. Just in case, I have included some code in the `usac_task_clean.R` script that should set the working directory to the file's location.
